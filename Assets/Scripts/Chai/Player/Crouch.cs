...
public class Crouch : MonoBehaviour
{
    ...
    public AudioSource footstepAudioSource;  // 脚步声音的 AudioSource
    public float normalFootstepVolume = 1.0f;  // 正常音量
    public float crouchFootstepVolume = 0.2f;  // 蹲下时的音量
    ...
    void LateUpdate()
    {
        if (Input.GetKey(key))
        {
            ...
            if (!IsCrouched)
            {
                ...
                // Lower footstep sound volume when crouched
        ...
        else
        {
            if (IsCrouched)
            {
                ...
                // Reset footstep sound volume to normal
                if (footstepAudioSource)
                {
                    footstepAudioSource.volume = normalFootstepVolume;
                }
            }
        }
    }
